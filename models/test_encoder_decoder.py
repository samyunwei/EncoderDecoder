...
class EncoderRNN(nn.Module):
    def __init__(self, embeddings, opts):
        ...
        self.gru = nn.GRU(self.hidden_size, self.hidden_size, self.n_layers,
                          dropout=opts.drop_rate, bidirectional=True)
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, embeddings, opts):
    #hidden_size, output_size, n_layers=1, dropout=0.1
        ...
        self.attn = Attn(attn_model, self.hidden_siz)
    ...
    def forward(self, sources, source_length, targets, targets_length):
        ...
        for t in range(self.max_target_len):
            decoder_output, decoder_hidden = self.decoder(
                targets[:,t,:], decoder_hidden, encoder_outputs
            )
            output_hiddens.append(decoder_hidden)

        return output_hiddens, None
